    ...
    for tc in tcs:
        ...
        if minor_display:
            ...
            if len(raw_lines) >= 2:
                ...
                seed_no_paren = re.sub(r"\([^)]*\)", "", seed_clean).strip()
...
def parse_obligatory_patterns(md_path: str) -> list:
    """obligatory_tc_patterns.md 의 표를 파싱.
    Returns: list of {screen_code, pattern_name, keywords, why}
      keywords: list of list — 외부 list = OR, 내부 list = AND
                예: 'A, B+C' → [['A'], ['B', 'C']]
    """
    ...
    for line in text.splitlines():
        ...
        if len(cells) < 4:
            continue
        sc, pname, kws, why = cells[0], cells[1], cells[2], cells[3]
        # 헤더/구분선 거르기
        if sc in ("ScreenCode", "------------") or set(sc) <= set("-"):
            continue
        if not sc or not pname or not kws:
            continue
        # 키워드 파싱: ',' OR, '+' AND
        or_groups = []
        for or_part in kws.split(","):
            or_part = or_part.strip()
            if not or_part:
                continue
            and_terms = [t.strip().lower() for t in or_part.split("+") if t.strip()]
            if and_terms:
                or_groups.append(and_terms)
        if or_groups:
            patterns.append({
                "screen_code": sc.upper(),
                "pattern_name": pname,
                "keywords": or_groups,
                "why": why,
            })
...
def detect_missing_obligatory_tcs(tcs: list, patterns: list) -> list:
    """TC 목록에서 의무 패턴 누락 검출.
    매칭 단위: 각 TC 의 (제목 + 소분류 + 사전조건 + 스텝 + 기대결과) 통합 텍스트
    Returns: list of {screen_code, pattern_name, why}  (누락된 항목만)
    """
    if not patterns:
        return []
    # 화면별 TC 그룹핑 (SuiteCode 기준)
    suite_to_tcs = {}
    for tc in tcs:
        tc_id = tc.get("id", "")
        m = re.match(r"^[A-Z]{2}-([A-Z][A-Z0-9\-]*)-\d+$", tc_id)
        ...
        # 멀티-suite (예: TRD-ORDR) 의 첫 토큰 사용 — pattern 의 ScreenCode 와 매칭
        suite_first = suite.split("-")[0]
        suite_to_tcs.setdefault(suite_first, []).append(tc)
    # TC 별 통합 텍스트 (소문자)
    def _tc_text(tc: dict) -> str:
        parts = [tc.get("title", ""), tc.get("minor", ""),
                 tc.get("given", ""), tc.get("when", ""),
                 tc.get("then", "")]
        return " ".join(p for p in parts if p).lower()
    missing = []
    for pattern in patterns:
        target_sc = pattern["screen_code"]
        target_tcs = []
        if target_sc == "*":
            # 전 화면 — 모든 SuiteCode 그룹별로 검사
            for suite, tcs_list in suite_to_tcs.items():
                target_tcs.append((suite, tcs_list))
        else:
            if target_sc in suite_to_tcs:
                target_tcs.append((target_sc, suite_to_tcs[target_sc]))
        for suite, tcs_list in target_tcs:
            # 이 SuiteCode 그룹의 TC 중 하나라도 패턴에 매칭되면 통과
            matched = False
            for tc in tcs_list:
                txt = _tc_text(tc)
                # OR 그룹 중 하나라도 통과하면 매칭
                for and_terms in pattern["keywords"]:
                    if all(term in txt for term in and_terms):
                        matched = True
                        break
                if matched:
                    break
            if not matched:
                missing.append({
                    ...
                    "keywords_display": " OR ".join(
                        " + ".join(t) for t in pattern["keywords"]
                    ),
                    "why": pattern["why"],
                })
    return missing
...
def build_obligatory_check(ws, missing: list, total_patterns: int) -> None:
    """의무 TC 누락 검출 시트.
    호출부에서 missing 이 0건이면 시트 생성 안 함.
    """
    ws.sheet_view.showGridLines = False
    headers = ["화면 코드", "누락 패턴", "예상 키워드 (참고)", "이유 (왜 분리해야 하는가)"]
    col_w = [13, 28, 38, 38]

    set_cell(ws, 1, 1,
             f"⚠ 의무 TC 패턴 누락 — 총 {len(missing)}개 / 검사 {total_patterns}개. "
             f"검토 후 TC 재생성 또는 spec 보강 필요 (자동 추가 X).",
             bold=True, bg=C_DARK, font_color=C_WHITE, size=11, align_h="left")
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=len(headers))
    ws.row_dimensions[1].height = 24
    ...
    for ri, m in enumerate(missing, 3):
        bg = C_ROW_A if (ri - 3) % 2 == 0 else C_ROW_B
        row_vals = [m["screen_code"], m["pattern_name"],
                    m["keywords_display"], m["why"]]
    ...
    last_col = get_column_letter(len(headers))
    ws.auto_filter.ref = f"A2:{last_col}{len(missing) + 2}"
    ws.freeze_panes = "A3"
...
def run_build(phase: str, tc_path, output_dir,
    ...
    # 의무 TC 패턴 누락 검출 (obligatory_tc_patterns.md 기반)
    try:
        # 프로젝트 폴더 경로 추정 — TC 의 첫 ID prefix(SC/SM) 로부터 프로젝트 추론
        # 또는 현재 작업 폴더 기준 ../tc-agent/projects/{name}/obligatory_tc_patterns.md
        possible_paths = [
            Path("/Users/myungjookim/_claude26/git-projects/tcgenerator/tc-agent/projects/supercycl/obligatory_tc_patterns.md"),
            Path(__file__).parent.parent / "projects" / "supercycl" / "obligatory_tc_patterns.md",
        ]
        patterns_path = None
        for p in possible_paths:
            if p.exists():
                ...
                break
        if patterns_path:
            patterns = parse_obligatory_patterns(patterns_path)
            if patterns:
                missing = detect_missing_obligatory_tcs(tcs, patterns)
                if missing:
                    ws_obl = wb.create_sheet("obligatory_tc_check")
                    build_obligatory_check(ws_obl, missing, len(patterns))
                    if verbose:
                        print(f"  → ⚠ 의무 TC 패턴 누락 {len(missing)}개 / 검사 {len(patterns)}개 — 'obligatory_tc_check' 시트 참고")
                elif verbose:
                    print(f"  → ✓ 의무 TC 패턴 검증 통과 (검사 {len(patterns)}개, 누락 0개)")
    except Exception as e:
        if verbose:
            print(f"  → 의무 TC 검증 스킵: {e}")
