...
기획팀이 릴리즈하는 폴더 형식 (`01_spec.md` + `02_ux_design.md` + `05_error-handling.md` + `scr/SCR-*.md`) 을 그대로 입력으로 받는다.

```
spec-folder/
├── 01_spec.md            ← 화면 목록 표 (대분류/중분류/화면 자동 매핑)
├── 02_ux_design.md       ← 디자인 토큰/컴포넌트 (system 캐시)
├── 05_error-handling.md  ← 에러 패턴/공통 정책 (system 캐시)
└── scr/
    ├── SCR-001.md        ← 화면별 상세 (상태 표/인터랙션/비고)
    ├── SCR-104.md
    └── ...
```

**핵심 효과**:
- **분류 단계 LLM 호출 0회** — `parse_screen_list_table()` + `extract_minors_from_screen_md()` 가 정규식만으로 분류표 자동 생성
- **화면별 1:1 LLM 호출** — `step_write_tc_per_screen()` + `call_claude_cached()` + cache_control (4 블록) → 시스템 프롬프트(정책+디자인+규칙+분류표) 1회만 캐시, 화면 N개 호출에 캐시 히트
- **45개 화면 처리 시 입력 토큰 ~98% 절감**
- **분류표 계층 정상화**: 대분류(영역) > 중분류(화면) > 소분류(세부 시나리오)
  - SCR md 의 `**상태 (Status)**` 표, `**에러 케이스**` 표, `**[dev] 인터랙션**` 항목, `**비고**` 의 `[정책]/[제약]/[접근성]` 마커를 자동 추출 → 화면당 8~12개 소분류 시드

### 🔄 버전 diff 모드

이전 spec 폴더와 신규 spec 폴더를 SCR 단위 SHA-256 해시로 비교 → 변경/추가/삭제/동일 분류 → 변경된 화면만 재생성.

검증: Supercycl v0.43 → v0.47 비교 결과 추가 0개 / 수정 11개 / 삭제 0개 / 동일 34개.
공통 변경 시 자동으로 전체 처리, 공통 미변경 시 24% 만 재생성 → **비용 76% 절감 가능**.

### ⏯ resume 지원

화면별 1:1 호출이 끊겼을 때 (네트워크/타임아웃/수동 중단) 이미 완료된 화면은 건너뛰고 미완료 화면만 이어서. `workspace/<sid>/per_screen/SCR-*.md` 에 화면별 결과를 즉시 저장 → 재진입 시 자동 식별.

### 🎨 Step 1 입력 화면 정식 레이아웃

**기존**: 개별 소스 추가 버튼이 메인 + 회색 점선 박스로 '구조화 spec 폴더 모드 (신규)' 가 보조 UI 처럼 배치
**신규**:
- ① 📁 구조화 spec 폴더 [권장] — 파란 그라데이션 + 1.5px 강조 테두리 + 기본 펼침
- ② 📝 개별 소스 입력 [임시 작업용] — 슬레이트 회색 + 기본 접힘 (아코디언)

추가:
- "최근 사용 ▾" 드롭다운 — 프로젝트별 최근 사용 spec 폴더 자동 제안 (최대 10개, 존재하는 경로만 필터)
- 카드 우상단 **"← 처음으로 (입력 변경)"** 버튼 — Step 2/Step 3 어느 시점에서도 새로고침 없이 입력 화면 복귀
- TC 생성 범위 placeholder 강화 — `SCR-102, 104, 106, 116` 같은 일괄 표기, `SCR-102~116` 범위 표기 안내

### 🎯 focus_area SCR 필터 — 관대한 인식

| 입력 형식 | 인식 |
|---|---|
| `SCR-102, 104, 106, 116` | ✅ 4개 모두 (앵커 모드) |
| `SCR-102~116` (범위) | ✅ 폴더 안 실제 존재 SCR 만 |
| `SCR-102 to 116` | ✅ 동일 |
| `SCR 102, 104, 106` (하이픈 없이) | ✅ |
| `주문 화면 SCR-104, 106 만 만들어줘` | ✅ (자연어 섞여도) |
| `SCR-007A, 007B` (영문 접미) | ✅ |

폴더 검증: 존재하지 않는 SCR ID 는 빈 set 반환 → 명확한 에러 메시지.

### 🛠 다수 안정화

**TC 품질**:
- max_tokens 8192 → 16000 (응답 잘림 방지)
- TC 4가지 카테고리 가이드 (UI 20% / 주요 35% / 예외 25% / 에러 20%) 강제
- 결과: 화면당 ~22개 → ~40개 수준 회복

**Excel 출력**:
- 컬럼 너비 하이브리드 정책 (FIXED + AUTO min/max) + Google Sheets 호환 1.4배 스케일
- 그룹 구분 행 항상 표시 + 중간 회색(#BDBDBD) 통일
- 소분류 셀의 `'A — B'` em-dash 연결을 줄바꿈으로 변환
- 분류표 중분류에서 SCR ID 제거 (화면 코드 컬럼과 중복 제거)
- 시트명에서 이모지 제거 (외부 프로그램 호환성)

**용어 정리**:
- 사용자 노출 텍스트 "도메인" → "대분류" 통일
- 변수명/내부 주석은 유지 (1단계 정책)

**버그 수정**:
- cache_control 블록 5→4개 (Anthropic API 제약)
- step_review() 반환값을 TC 본문으로 잘못 사용한 버그 (Excel 폴백 시트만 생성됨)
- step_build_excel() 인자 누락 (total_tc/min_tc)
- done 이벤트 페이로드 키 불일치 (NaN KB 표시 문제)
- spec_folder 타입이 'undefined' 카드로 표시되던 복원 버그
- 최근 사용 드롭다운 렌더의 따옴표 escape SyntaxError

### 📊 비용·속도 비교 (Supercycl 45개 화면 기준)

| 모드 | 분류 LLM | 화면 LLM | 캐시 효과 | 첫 호출 | 캐시 히트 호출 |
|---|---|---|---|---|---|
| 기존 (raw concat) | 2~3회 | 1회 (전체) | 없음 | ~$0.5 | — |
| **v0.10.0 (구조화 spec)** | **0회** | **N회** | 입력 토큰 ~98%↓ | ~$0.3 (1화면) | ~$0.05 (이후 화면) |
| v0.10.0 + diff 모드 | 0회 | 변경된 N개만 | 동일 | 추가 절감 76% | — |

### ⚠️ 호환성

- 기존 raw_text concat 모드(`/start`)는 **그대로 유지**. 다른 프로젝트나 단발 작업에 계속 사용 가능.
- 기존 세션 복원 시 `spec_folder` 타입 소스는 자동으로 새 입력 칸에 라우팅.
- 변수명(`domain`, `extract_domains` 등)과 내부 주석은 1단계 통일에서 의도적으로 유지 — 외부 노출 X.
